...
| **PostgreSQL** | 控制面元数据、审计、服务目录（M2+） | **建议现在就起**，便于后续 `platform-api` 直接连 |
...
3. 为 `platform-api` 增加 **PostgreSQL ping** 与 **配置化 DSN**（`DATABASE_URL`），在 Compose 里加 `depends_on: postgres` 的侧车或后续再挂同一网络。
